...
internal data class LiveReservationDiagnosticConfig(
    ...
    companion object {
        ...
        fun from(environment: Map<String, String>): LiveReservationDiagnosticConfig? {
            if (environment[ENABLE_FLAG] != ENABLE_VALUE || environment[CONFIRM_FLAG] != CONFIRM_VALUE) return null
            ...
            return LiveReservationDiagnosticConfig(cardNumber, password, tilcod, pickup)
        }
    }
}
...
internal suspend fun runLiveReservationDiagnostic(
    ...
    try {
        val reservedBefore = session.fetchReservations().any { it.tilcod == config.tilcod }
