...
#### 予約が業務的に拒否されたときの応答（2026-07-27、実機で確認済み）

**確定POSTの直後応答からは、現行のパーサで拒否理由を取得できない。応答は予約確認画面の再表示である。**

サイト全体に理由が存在しないという意味ではない。ブラウザではダイアログが表示されることが所有者の
観察で分かっており、確定POSTの直後には `WOpacCommonBackAction.do` の要求も存在する。
現時点で言えるのは「確定POSTの直後応答を現行の抽出範囲で見る限り、理由が見当たらない」ことまでである。

予約上限に達しているアカウントで `ONE PIECE 巻115`（コミック）の確定POSTを送ったときの実測。
**全体20件の上限とコミック4件の上限のどちらに当たったかは未確定である**:

- HTTPステータス200
- 応答本文は `gamenid=tiles.WYoyConfirm` の予約確認画面（診断の画面分類も `reservation-confirmation`）
- `div#messages` は空。`messages` カテゴリに何も出ない
- `h1`/`h2` の見出しは「予約確認」。エラー表示に使われそうな要素にも拒否理由は無く、
  広告・閉室のお知らせ・対象書誌の書誌情報だけが並ぶ

したがって**現行の実装が見ている範囲では、拒否理由を取得できていない**。成功と失敗を確実に区別できるのは
予約一覧の `tilcod` 照合だけである。現在の実装（確定POSTを送ってから予約一覧で照合する）は、この点で
妥当な設計だったことになる。理由の取得が必要になった場合は、確定POST直後の
`WOpacCommonBackAction.do` までを含めた調査が別途必要である（未着手）。

予約状況一覧が20件でもページングされないことを実測済み（§9末尾参照）であるため、この照合に
偽陰性は起きない。よって「確定POST後の応答が確認画面（`StayedOnConfirmation`）」かつ「予約一覧に
対象`tilcod`が無い」の組み合わせは、サイトが拒否したと断定してよい。アプリはこの組み合わせを
`FailureReason.REJECTED_BY_SITE`として扱う（理由は不明なため、UIでは断定的な理由を表示しない）。

重複予約の場合はalert文言「予約済の書誌があります。予約できません。」が出ると6.3に記録があるが、
上限超過ではそれも出ない。文言による判定に依存してはならない。

**2026-07-27追記（所有者からの情報。ブラウザでの観察）**: 実サイトでは予約実行時に、完了または
上限超過の旨が**ダイアログ（ポップアップ）で表示される**。上限超過の場合は、現在の予約数と今回
予約しようとした冊数の合計が20を何冊上回るかが示される。**空きがあっても1冊も予約されない**
（部分的に受け付けることはしない）。

この情報は、上記の実測「確定POSTの直後応答からは理由を取得できない」と食い違って見える。
**食い違いの原因は診断側にある可能性が高い。** 当時の診断は `div#messages li` だけを見ており、
`alert(...)` などスクリプト内に直接書かれた文言を拾えなかった（2026-07-27に抽出範囲を広げたが、
広げた後の応答での再確認はまだ行っていない）。ブラウザのHARでは確定POSTの直後に
`POST WOpacCommonBackAction.do`（`…MessageUtil.CONFIRM_DIALOG_SEND_REDIRECT=true` を伴う）が
あり、「ダイアログを表示し、OKされたら戻る」作りを示唆している。抽出範囲を広げて再確認する必要がある。

**自動予約の設計への影響**: 上限超過時に部分的な受け付けが無いということは、複数冊をまとめて送る
導線では「1冊でも超えれば全滅」になる。アプリの直接予約は1冊ずつ確定POSTを送るため各冊が独立して
成否を持つが、この違いは把握しておくこと。
