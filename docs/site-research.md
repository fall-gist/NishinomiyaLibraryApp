...
### 予約取消は2段階であり、実サイトでの取消は未完成（実測、2026-07-27）

上記までの実装後、実サイトへ実際に`WOpacUsrRsvCancelAction.do?mngFlg2_handan=1&kbnchgflag=1`を
POSTして初めて次が判明した。**この記述までの実装(既知拒否語による判定)は誤りであり、修正済み。**

- **応答は予約状況一覧の画面であり、ダイアログ文言 `予約の取消を行います。よろしいですか？`
  （`lbConfirm`呼び出し）を含む。この時点では取り消されていない**ことを、取消後の一覧に
  対象がそのまま残っていることで実測確認した。すなわちサイトの取消操作は
  「確認ダイアログ表示→OK→実際の取消送信」の2段階であり、アプリが送っている1回のPOSTは
  1段階目（確認表示）にしか到達していない。
- **OK後に何が送信されるかは未特定である。** 共通JS `lbwebdialog.js` の `lbConfirm` は
  `window.confirm()`をラップして真偽値を返すだけの汎用関数であり、OK後の実際の送信内容
  （別アクションへのPOSTか、同一フォームへの再送か、追加フィールドの有無）はページ固有の
  スクリプト側が持っている。診断ログの強化（本体抽出条件へ`confirm(`/`lbConfirm(`を追加、
  トップレベルの`confirm`呼び出し文の抽出を追加）を行ったが、本実装時点ではOK後の遷移先は
  未確認のままである。
- **拒否判定の欠陥**: 旧実装は「できません」「越えています」という一般語で拒否を判定して
  いたが、これは**全ページ共通で埋め込まれているJS定数**
  （例: `仮パスワードでは利用できません。パスワード変更を行なってください。`）にも一致してしまい、
  取消と無関係の応答でも誤ってRejected扱いになる欠陥があった。この判定は削除し、実測した
  具体的な文言（`取消を行います`を含むかどうか）だけで「確認画面が返った(`ConfirmationRequired`)」
  ことを判定するよう修正した。
- **結論**: 予約取消は本実装時点(2026-07-27)で未完成である。アプリは1段階目の確認画面を
  検出して「取消を完了できませんでした」として打ち切ることはできるが、実際にサイト上で
  予約を取り消すには至っていない。OK後の送信内容の実測が今後の課題として残る。
