...
- 2段階目のReferer/Originは1段階目の応答ページ（`WOpacUsrRsvCancelAction.do?mngFlg2_handan=1&kbnchgflag=1`）
  由来とする。予約確定(directReserve)と同じ安全策（排他区間・exactly-once・自動リトライ無効）を
  踏襲し、2段階目の応答にも確認ダイアログ文言が含まれる場合は`ConfirmationRequired`のままとする。
- 実装（`LicsXpReservationSession.cancelReservation`）は、1段階目の応答に確認ダイアログ文言を
  検出した場合だけ上記の2段階目を自動的に送り、いずれの段階でも確認ダイアログが返らなければ
  取消後の一覧照合で成否（`Cancelled`/`IndeterminateAfterPost`）を判定する。ユニットテスト
  （`ReservationGatewayTest`、MockWebServerのみ）で本文の順序・Referer/Origin・POST回数
  （取消POSTは1段階目・2段階目の合計2回まで）を検証済み。
