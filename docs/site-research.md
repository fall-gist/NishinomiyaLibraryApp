...
#### 確定POSTが受け付けられなかったときに観測された応答（2026-07-27）

**予約上限を意図的に超過させる試験は未実施である。** 以下は、ある資料（`ONE PIECE 巻115`）への
確定POSTが結果として成立しなかったときの観測であり、**なぜ受け付けられなかったかは不明**である。
上限超過であったという裏付けは無い。原因の候補（全体20件の上限、コミック4件の上限、資料固有の
予約不可、その他）はいずれも未検証である。
...
したがって**現行の実装が見ている範囲では、受け付けられなかった理由を取得できていない**。
サイト全体に理由が存在しないという意味ではない。ブラウザではダイアログが表示されることが所有者の
観察で分かっており、確定POSTの直後には `WOpacCommonBackAction.do` の要求も存在する。
成功と失敗を確実に区別できるのは予約一覧の `tilcod` 照合だけである。現在の実装（確定POSTを送ってから予約一覧で照合する）は、この点で
