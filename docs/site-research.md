...
> 2026-08-03追記: このHAR確定値を用いる二段階非表示は、既存の`cancelReservation`内部へ接続済みである。
> 2026-08-04、実サイトでのアプリ取消から非表示までの一連成功を所有者が確認した。通常テスト・CIから
...
- ダイアログでOKが選ばれると、共通のJS関数が`OK_CODES_NAME = "okCodes"`という変数名で
  ...
  （クエリなし、絶対パス）を**JavaScriptの代入で実行時に設定してから**`submit()`する（2段階目）。
  この`OK_CODES_NAME`/`prevRequestForm`という命名は、予約取消・予約非表示で確認済みの
  共通確認ダイアログ機構と同一である。
- **【2026-08-05訂正・重要】`prevRequestForm`のHTMLタグには`action`属性が存在しない。**
  実測した生タグは`<form name="prevRequestForm" method="post">`であり、属性は`name`と`method`だけである
  （同じ応答内の`LBFormMF`・`LBForm`も同様に`action`属性を持たない）。送信先はすべて上記のJS代入で
  決まる。初版の「2段階目のactionは常に明示的」という記述は、**JSが明示的に代入する**という意味であって
  **HTML属性として書かれている**という意味ではない。静的HTMLパーサ（Jsoup）で`form.attr("action")`を
  読んでも常に空文字列であり、これを必須条件にすると実サイトでは必ず停止する。予約取消の
  `prevRequestForm`実測（本書11回目、`action=(empty)`）とも一致する挙動である。
- **【2026-08-05訂正】`prevRequestForm`の隠しフィールドは、1段階目の「query + body」の全パラメータと
  一致する（DOM順・同名重複込み）。** 実測は次のとおり。
  - 1段階目 query: `mngFlg1_handan`（1件）
  - 1段階目 body: `schkflg`・`allschkflg`・`hash`・`islogin`・`btnflg`・`checkflag`・`booklistvalue`・
    `commntvalue`・`btnflg`・`checkflag`・`returnid`・`gamenid`・`para`・`mngFlg1`・`sortkeyvalue`・
    `sortDefKey`・`booklist`（17件、`btnflg`と`checkflag`が各2回）
  - `prevRequestForm`: 上記18件（先頭が`mngFlg1_handan`、以降はbodyと同じ顔ぶれ。多重集合として
    query+bodyと完全一致することを実測で確認済み）
  - 2段階目 body: 上記18件＋末尾`okCodes`＝19件
  初版は`prevRequestForm`をbody（17件）とだけ突き合わせて「完全一致する」と書いており、
  `mngFlg1_handan`の1件を見落としていた。**実装で多重集合の一致検査を行う場合、期待値はbodyだけでなく
  query+bodyでなければ必ず不一致になる。** フォーム名（「直前のリクエストのパラメータを格納したフォーム」）
  の趣旨とも、query+bodyという解釈のほうが整合する。
...
#### 延長成功時の一覧の変化（2026-08-05、HARの行単位比較で実測）

1段階目応答（＝延長確定前の一覧）と2段階目応答（＝延長後の一覧）を、書誌詳細リンクの資料コードを
キーに行単位で突き合わせた結果である。**資料コードの実値は記載しない。** 対象資料を「対象行」、
それ以外を「他の行」と呼ぶ。

| 観測項目 | 結果 |
|---|---|
| 一覧の行数 | 送信前後とも同じ（行は**消えない**） |
| 対象行の返却期日 | **後ろの日付へ変化した**（貸出日+14日 → 延長実行日+14日。1件の観測からの**推定**） |
| 対象行の延長ボタン | **消えた**（`extend(...)`が無くなる） |
| 他の行の返却期日 | すべて不変 |
| 他の行の延長ボタン | 有無とも不変 |
| **行の並び順** | **変わった。** 対象行が一覧の末尾へ移動した（返却期日の昇順と整合する動き） |

ここから確定する実装上の要点:

1. **成否判定の主判定（対象行の返却期日が後ろへ変化したか）は、実データで機能する。**
2. **延長成功後、対象行は「延長ボタンを持たない行」になる。** したがって、送信後の照合で
   「延長ボタンのある行」だけを対象にすると、**成功したときに限って対象行を見失う**。
   送信後の一覧はボタンの有無に関わらず全行を保持して照合すること。
3. **行の位置で対象を追ってはならない。** 並び順が変わるため、資料コードで照合する必要がある。
   位置で突き合わせると、今回の実データでは対象行と別の資料の行を比較してしまい、
   「返却期日が変わっていない」と誤判定する（実際に本調査の初回集計でこの誤りが起きた）。
...
- 延長直後に対象行のボタンが消えることは実測したが、**時間を置けば再び延長できるようになるのか、
  それとも1回限りなのか**は未確認。
...
**`click_delBook`・`click_uplist`・`click_ok`のいずれにも`lbConfirm`が無い。** クライアント側の
関数は確認を挟まずに送信する。`click_return`（キャンセル）にだけ「本棚の編集処理を中断します。」の
`lbConfirm`がある。

**ただし、所有者が実サイトで本棚の作成・削除・更新のいずれでも確認画面が出ることを確認している
（2026-08-08）。** クライアント関数に確認が無いことと合わせると、**確認は1段階目POSTの応答ページ側に
あり、予約取消・貸出延長と同型の二段階POST**であると考えられる。

> **この系統のサイトでは、状態を変える操作に確認画面が挟まるのが全体の共通仕様である可能性が高い**
> （所有者の観察、2026-08-08）。断定はしないが、**新しい書き込み機能を調べるときは
> 「二段階である」を初期仮説に置き、一段階で完了すると仮定した実装を先に書かないこと。**
> 予約確定・予約取消・予約非表示・貸出延長がいずれも二段階だった事実とも整合する。
...
- 各POSTの応答構造、成功時の画面、確認段階の**具体的な構造**（確認画面が出ること自体は
  所有者が作成・削除・更新のいずれでも確認済み。`prevRequestForm`・確認コードの有無は未採取）
- 2段階目の送信先とbody（予約取消・貸出延長と同型かどうか）
...
| 資料削除 | `WOpacSdiBookDelAction.do?flg=1` | 二段階 | 本棚からこのタイトルを削除します。よろしいですか？ | `OPACSDI033` |
| 本棚削除 | `WOpacSdiBookListDelAction.do?delflg=1` | 二段階 | このリストを削除します。よろしいですか？ | `OPACSDI010` |
...
資料削除・本棚削除の確認コードは2026-08-09に確定した。専用テスト棚を対象に、各操作を独立した
