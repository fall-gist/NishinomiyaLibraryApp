...
したがって**アプリ側で「なぜ拒否されたか」をサイトの応答から知る手段は無い**。成功と失敗を区別できるのは
...
この情報は、上記の実測「拒否理由の表示は無い」と食い違う。**食い違いの原因は診断側にある可能性が
高い。** 現在の診断は `div#messages li` だけを見ており、`alert(...)` などスクリプト内に直接書かれた
文言は拾えない。ブラウザのHARでは確定POSTの直後に
