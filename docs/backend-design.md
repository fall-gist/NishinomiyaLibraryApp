...
> 送信を`cancelReservation`内部へ接続済みである。実サイトでのアプリ取消から非表示までの一連成功は未実施。
...
2026-08-03時点では、Stage 15 HARで確定した二段階プロトコルを既存取消フローへ接続済みである。
実サイトでのアプリ取消から非表示までの一連成功は未実施であり、診断は通常テスト・CIから除外した専用Gradleタスク
