...
- 参照系のバックエンド(データ層)は実装・実サイト検証まで完了済み(handoff.md 参照)。
  予約バックエンドは実装済みだが、予約カート画面・予約導線は未実装
...
- 既存スコープで残る未実装はホームの「きょうの図書館」セクションのみ。加えて、
  **予約カート画面と予約導線は未実装**である（予約バックエンドの契約・安全処理は実装済み）
...
| 予約カート(ハンバーガー) | 3.10 | — | **UI未実装**。メンバー別の候補確認・受取館選択・最終確認・結果表示 | `ReservationCartRepository` |
...
## 確定済みデザイン: 予約導線と予約カート(2026-07-22確定・UI未実装)
...
実装済みの`ReservationCartRepository.cartItems()`から構成する。
