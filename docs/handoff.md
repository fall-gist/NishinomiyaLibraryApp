...
### 5. 貸出中画面からの貸出延長（2026-08-04、実サイト通信を確認し設計中）
...
- `docs/spec.md`§3.12へ機能要件を追加した（設計中・未確定の位置づけ）。技術設計§9に所有者確認が
  必要な論点（UI導線の範囲、拒否理由を区別しない初期実装でよいか、共通書込ゲートに参加させないか）
  を3点挙げている
- 書き込み操作であるため、上記論点の確認後に実装へ進む。専用ゲートウェイへの隔離、送信一回限り、
  成否不明時は再送せず照合で解決、という不変条件は他の書込機能と同じく維持する
