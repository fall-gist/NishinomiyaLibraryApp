...
### 6. 予約取消に続けて「非表示」まで自動で処理する（2026-07-28追記。未着手・未設計）

**所有者の想定は「UIに非表示ボタンを置く」ことではなく、予約取消が成立したあとアプリが続けて
非表示まで処理してしまうこと。** 今は着手せず、構想として記録するに留める。
...
- サイト側の導線は既知: 画面内アクション`yoykHihyoji:WOpacUsrRsvHiddenAction.do?mngFlg2_handan=1`
  （`docs/site-research.md`§9のアクション一覧より）。ただし**送信本文・2段階目の有無・応答は未実測**である。
  取消と同様に、ブラウザの成功通信を採取してから実装すること（§禁止事項10）。
- 現在の実装は非表示ボタンの**コード値（`yoykHihyoji('...')`の引数）を保持していない**。判定に必要なのが
  有無だけだったため。実装時はパーサで抽出して`Reservation`へ持たせる必要があり、`cancelCode`追加と
  同じくRoom v7→v8のマイグレーションを伴う。
- 設計上の論点（未決）: 非表示は取消とは別の書き込みであり、取消成功後に**自動で**続けるなら、
  利用者の1操作で2つの書き込みが走ることになる。禁止事項1（書き込みは機能ごとに設計と承認）に照らし、
  所有者の明示承認が要る。取消は成功したが非表示に失敗した場合の扱い（成否表示、再試行の可否）も未決。

### 7. 各画面でスワイプダウンによる手動更新（2026-07-28追記。未着手・未設計）

**これは新機能ではなく、仕様にあるのに実装できていない項目である。** `docs/spec.md`§3.7に
「手動同期: プルリフレッシュ」と既に書かれているが、現在の実装はホーム画面の「いますぐ同期」
ボタンだけであり、スワイプダウン→リリースによる更新機構はどの画面にも無い。

- 所有者の意向は、ホーム画面に限らず**各画面**で一般的なプルリフレッシュを使えるようにすること。
- 現状の手動同期の入口は`HomeScreenController`経由の`syncAll(SyncTrigger.MANUAL)`だけである。
  複数画面から呼べるようにするにあたり、同期状態（実行中・最終同期時刻・結果）をどこで保持して
  各画面へ配るかを設計する必要がある。`syncAll`自体は`syncMutex`で直列化されているため、
  複数画面から同時に呼ばれても二重実行にはならない。
- 手動更新の5分クールダウンは2026-07-28に所有者の判断で撤廃済みであるため、プルリフレッシュが
  クールダウンで無反応になる問題は起きない。ただし**リクエスト間500msの制限
  （`LicsXpSession.MINIMUM_REQUEST_INTERVAL_MILLIS`）は残っている**ので、連続実行しても
  サイトへのリクエストが詰まることはない。
- 未決: 対象とする画面の範囲（ホーム／貸出中／予約中／本棚／読書記録のどこまでか）、
  同期中インジケータの出し方、失敗時の見せ方、書誌詳細ポップアップが開いている状態での扱い。

なお`docs/spec.md`§3.7の自動同期（毎日指定時刻に1回・時刻は設定変更可能）は
`SyncScheduler.createDailySyncWorkRequest`で実装済みであり、仕様との乖離は無い（2026-07-28確認）。
