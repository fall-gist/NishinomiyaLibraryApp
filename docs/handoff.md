...
- 関連する未検証事項: 予約上限に達したときのサイト応答（§6.4）。上限超過時に確定POSTが何を返すかが
  分かっていないため、`FailureReason` のどれに落ちるかも未確定である。
...
**2026-07-27追記: 上限に達したときのサイト応答は実機で確認済みとなった。** 確定POSTの直後応答は
予約確認画面の再表示であり、現行の抽出範囲では拒否理由を取得できない（site-research.md §6.4）。
サイト全体に理由が無いという意味ではない。あわせて予約状況一覧が20件でも
ページングされないことも確認できたため、「確定POSTの応答が確認画面」かつ「予約一覧に対象が無い」
ならサイトが受け付けなかったと断定してよい。この判定は実装済みで、失敗理由も
「図書館サイトが予約を受け付けませんでした（予約上限に達しているなどの理由が考えられます）」と
表示するようにした。自動予約でも、拒否された資料をその場で失敗として確定できる。
...
### 業務的拒否の解決（2026-07-27実装）

上記「予約上限の扱い」で未検証としていた拒否時のサイト応答を実機確認した結果（site-research.md §6.4）、
確定POSTの直後応答は、現行パーサが取得できる拒否理由を含まず予約確認画面を再表示した。また予約状況一覧は
予約上限20件でもページングされないと実測した（同 §9）。現行サイト・検証アカウントでは、
1ページ目の一覧照合で対象を見落とさないことを確認済みである。ただし、上限値と非ページングを
...
**注意: このdry-runで使ったアカウント（環境変数に設定されているもの、予約19件）と、上限超過の
確認に使ったアカウントは別である。** 上限超過テストのアカウントは全体上限20件に達していた。
`ONE PIECE 巻115` の拒否がどの制限（全体20件／コミック4件）に当たったものかは**未確定**である。
