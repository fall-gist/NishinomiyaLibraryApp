...
### 6. 予約取消に続けて「非表示」まで自動で処理する（2026-08-03接続実装済み・実サイト一連確認は未実施）
...
`cancelReservation`内へ接続済みであり、実サイト通信調査は一連のライブ確認としてのみ残る。
...
- サイト側の導線はHARで確認済み: `yoykHihyoji`は固定stage1 query付きPOST、検証済み
  ...
  実サイトでアプリ取消から非表示までを通した一連成功は未実施である。
...
- **次に最初に行うこと**: 所有者が非表示してよい取消済み1件について、ブラウザまたは明示承認付き診断で
  実サイト一連成功を1回確認する。method、URL、全フォーム項目、Referer/Origin/Cookie、redirect、確認段階、
  応答、送信後一覧消失を確認し、詳細設計§9の反証条件に当たらないことを確定する。
...
- **接続済み**: 既存3経路の予約取消UIを変更せず、`yoykHihyoji`の二段階送信を取消成立後の内部処理へ接続した。実サイト一連確認は未実施。
...
重大・中・低の指摘なし。実サイトでのアプリ取消から非表示までの一連確認だけが未実施である。
