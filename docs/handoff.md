...
そのうえで、**上限に達したときのサイト応答は依然として未検証である**。必須の前提ではなくなったが、
確認しておくと次の点が改善する。

- 失敗理由をユーザーへ正しく伝えられる（現状は上限超過でも一般的な文言になる可能性がある）
- 自動予約が毎日同じ資料へ無駄な確定POSTを繰り返すのを避けられる（上限だと分かればその日は打ち切れる）

確認方法は、上限に達しているアカウントで予約を1回試し、診断ログの `messages` カテゴリを見るだけである。
