...
**2026-07-27時点: 予約上限を意図的に超過させる試験は未実施である。** 上限超過時にサイトが何を
返すかは分かっていない。

一方、理由は不明ながら**確定POSTが受け付けられなかった事例は観測している**（site-research.md §6.4）。
その応答は予約確認画面の再表示で、現行の抽出範囲では理由を取得できなかった。あわせて予約状況一覧が
サマリ件数と一致することも確認できたため、「確定POSTの応答が確認画面」かつ「予約一覧に対象が無い」
場合をサイトが受け付けなかったものとして扱う判定を実装した。失敗理由は
「図書館サイトが予約を受け付けませんでした（予約上限に達しているなどの理由が考えられます）」と
表示する。理由を断定していないのは、実際に何が起きたか分かっていないためである。
...
**注意: 予約上限を超過させる試験は未実施である。** `ONE PIECE 巻115` が受け付けられなかった事例は
観測しているが、その原因は不明であり、上限超過であったという裏付けは無い。上限超過時の挙動を
確かめるには、対象アカウントを上限まで埋めたうえで、さらに1件の確定POSTを試す必要がある。
