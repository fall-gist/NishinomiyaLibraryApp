...
`ReservationCartRepository`を利用できる。予約カート画面・検索/詳細からの導線は未実装である。
...
- **未実装**: 予約カート画面、検索/書誌詳細からの予約導線、受取館選択・最終確認・結果表示
