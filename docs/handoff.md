...
## 主担当AIへの進行指示（2026-07-27レビュー反映、2026-08-05に項目13〜15を追加）
...
### 5. 貸出中画面からの貸出延長（2026-08-05、段階1〜6完了・段階7＝実サイト確認のみ未実施）
...
- 受入条件は`docs/design/loan-extension.md`§12。所有者が延長してよい貸出資料1件を指定し、
  アプリの通常操作（貸出中一覧の「延長」→アプリの確認ダイアログ→確定）を**1回だけ**実行する。
- 反証条件（対象が一意特定できない、想定外のaction、返却期限が変化しない）に該当したら
  **追加POSTをせず設計を改訂する**。
- ライブ診断`liveLoanExtensionDiagnostic`は登録済み・**未実行**。必要な環境変数は
...
- 未確認のまま残る事項（§2）は段階7でも解消しない可能性が高い: 拒否時の応答構造、
  延長後に再度延長できるか、`mngcod`の安定性。**拒否例が実測できるまで`FailureReason`の
  細分類を作らないこと**（§9.2）。
- 実サイトで抽出方式（確認コード・送信先）が問題なく動くことを確認できたら、
  **予約取消側の同型2件（据え置き中）へ横展開してよい。それ以前に触らないこと。**
