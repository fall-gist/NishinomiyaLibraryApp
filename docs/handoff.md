...
収録実行が1回で設定状態の印がないため、マスターOFFと個別ルールOFFを通信レベルで個別に
立証したものではない。実機上のHilt Singleton同一性は段階13の待機表示試験まで未検証である。
...
対象テストと全単体テスト554件は成功し、独立したSolレビューでは重大・中程度の指摘なし。低重大度として、Compose上の
リンク表示・クリック、0人の実表示、ブラウザ不在時の自動UI試験が未整備であり、CI APKでの実機確認へ残す。
