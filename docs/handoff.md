...
次は段階12の実機回帰を行う。確認対象は、マスターOFFでPOSTされないこと、OFFルール見送り、
メンバー上限時のフォールバック、確保済み通知→ホーム→詳細→予約一覧の導線、実機上のHilt Singleton
同一性である。段階8以降の自動予約経路による実サイト通信は未検証であり、対象資料と副作用を
所有者が明示承認した手動診断以外では実行しない。
