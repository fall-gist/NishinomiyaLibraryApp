...
### 4. 新着資料からキーワード一致で自動予約する（2026-07-29要件・設計確定、段階1〜9完了）
...
次は段階10のAndroid通知とホーム画面への遷移を実装する。設定・履歴UIは段階11であり、
段階8以降の自動予約経路による実サイト通信と実機上のHilt Singleton同一性は未検証である。
