...
## 通知（2026-08-06、端末での受信を確認済み・**権限要求フローが未実装**）
...
### 未解決（重要）: `POST_NOTIFICATIONS`のランタイム権限を要求するコードが無い

`AndroidManifest.xml`に権限宣言はあるが、**`requestPermissions`／`ActivityResultContracts`等の
要求コードがアプリのどこにも存在しない**（全ソースをgrepして0件）。`targetSdk = 35`のため
Android 13以降ではユーザーの許可が必須であり、許可が無いと`AndroidNotificationSink.canPost()`が
`false`を返して投稿せずに終わる。

**今回受信できたのは、所有者が端末の設定アプリから手動で通知をオンにしたためである。**
アプリからは一度も権限ダイアログを出していない（所有者確認済み）。したがって
**アプリを入れ直した場合や別の端末では、通知は届かない。**

設計文書にも権限要求に関する記述は無く（`spec.md`に自動予約の前提として「通知権限は必須にしない」と
ある1箇所のみ）、意図的な省略ではなく**設計漏れ**と判断した。

**次に通知へ手を入れる際は、まずこの権限要求フローを実装すること。** 置き場所
（初回起動時／設定画面の通知セクション／通知トグルをオンにした瞬間）は未決定である。
