...
- 2026-07-19時点でroot環境の`testDebugUnitTest`は81件成功、`assembleDebug`も成功済み。チェックポイント修正後は再実行して確認する。
- 実サイトの読書履歴取得は、認証情報がこの環境に設定されていないため未実行。`scripts/fetch-fixtures.sh`には初回`usrread`取得を追加したが、実行時は取得内容の個人情報マスキング確認が必要。
