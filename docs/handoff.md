...
1. **書き込みの許可範囲**: **2026-07-26にリポジトリ所有者の判断で、従来の全面禁止を撤回した。**
   ...
   - 書き込みは引き続き専用ゲートウェイに隔離し、exactly-once・再送禁止・失敗時の照合という
     既存の不変条件を守る
   - **`docs/spec.md` には撤回前の記述が残っている**（自動予約をしない旨など）。各機能の設計時に
     spec.md の該当箇所も同時に改めること。改めるまでは spec.md の記述が形式上の正である
...
**予約上限に達した状態でのサイトの応答が未検証である（§6.4）。** 自動予約では上限到達が常態に
なるため、これが不明のままでは「上限で断られた」のか「不具合」かを区別できない。上限に達している
アカウントで予約を1回試し、診断ログを採れば確定する。**これを先に済ませること。**
