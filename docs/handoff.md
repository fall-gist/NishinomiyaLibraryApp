...
##### 予約取消側への横展開（2026-08-06実装済み・**実サイト未確認**）
...
**未解決（重要）**: **実サイトでの取消成功は未確認**。予約取消stage1応答の完全なHTMLを保持して
いないため（実測物はJSフラグメントのみ）、「HTML全体で`okArray`への代入が1件」は未検証である。
方針Aはfail-closeのため、この見込みが外れると**全テストが緑のまま予約取消が成立しなくなる**
（進行指示14の形）。**所有者の実機確認が済むまで完了扱いにしないこと。**

**既知の弱点（低）**: 「抽出した確認コードが実際に送信される」という不変条件はテストで守られて
いない。`buildForm()`をハードコードに戻しても全テストが緑のままである（管理セッションが破壊で確認）。
現状の安全性に穴はない（`parse`が想定値と照合してfail-closeするため`OPACUSR001`以外は送られない）が、
将来この照合を緩めるとリテラルが復活したときに誤った値を送り得る。
`ReservationCancelConfirmationForm`を直接構築して`buildForm()`の出力を固定する1件で塞げる。
**この弱点は独立レビューでは検出されず、管理セッションの破壊検証のみが検出した。**
