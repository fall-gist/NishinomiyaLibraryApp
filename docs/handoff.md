...
17. **ローカル検証は単独で実行し、複数セッションで同時にGradleを走らせないこと。**
    （2026-08-13追加。実際に踏んだ）管理セッションと実装セッションが同時に`testDebugUnitTest`を回し、
    `mergeDebugResources`・`transformDebugClassesWithAsm`で`Unable to delete directory`が多発して、
    双方ともテストに到達できなかった。**「8回中6回失敗」という誤った観測もこれが原因**である。
    実装セッションへ検証を任せた後に管理セッションが引き継ぐ場合は、**先に停止を指示し、`./gradlew --stop`と
    Javaプロセスの確認で実行中のビルドが無い状態を作ってから**開始する。「引き継ぐ」と宣言するだけでは
    相手は止まらない。ローカル実行には`JAVA_HOME`（Android Studio同梱のjbr）と`ANDROID_HOME`の
    明示指定が要る（§開発環境の注意）。
18. **`MockWebServer`で通信断を再現するときは`SocketPolicy.DISCONNECT_AFTER_REQUEST`を使うこと。**
    （2026-08-13追加）`DISCONNECT_DURING_REQUEST_BODY`は送信中の任意の位置で切断するため、サーバが
    リクエストを`RecordedRequest`として記録し終える前に切れることがあり、`server.requestCount`や
    POST回数の固定値アサーションがCIの遅い環境で不安定になる。本番側は`retryOnIOException=false`で
    IOExceptionの発生位置を区別せず一律`LibraryError.Network`として扱うため、**切断位置を変えても
    「通信断→再送しない」という検証内容は変わらない。**
19. **CIが失敗したら、まず落ちたテスト名を確認し、§「既知のフレーキーテスト」と照合すること。**
    （2026-08-13追加）**毎回違うテストが落ちる**、**`docs/`だけの変更で落ちる**といった兆候は、
    機能の退行ではなくテストの実時間依存を疑う。ただし**既知だからと放置しないこと。**
    赤が常態化すると本物の退行を見逃す。フレーキーを直すときは、必ず**修正前の失敗率を先に測り**、
    修正後と比較して改善を示すこと。ベースラインなしに直すと悪化させても気づけない
    （実際に、削除したアサーションが隠れた同期点として機能しており、失敗率を15回中13回へ悪化させた）。
20. **実測フィクスチャを追加するときは、`<script>`内の変数まで個人情報を検査すること。**
    （2026-08-16追加。実際に流出させた）本棚削除のHARからフィクスチャを作った際、
    **利用カード番号がHTML本文ではなくページ内JSの`var usrcod = '…'`に埋め込まれていた**ため
    見落とし、**PUBLICリポジトリへプッシュした**。本文の目視や表形式の検索では見つからない。
    追加前に必ず次を実行し、0件を確認すること。
    - `grep -rE "usrcod\s*=\s*'[0-9]+'" <対象ファイル>`（既存フィクスチャは`9999999`へマスク済み）
    - `hash`等のトークン、氏名、メールアドレス、7桁以上の数字列
    履歴に入った場合、`force push`だけではGitHub側にダングリングオブジェクトが残りうる。
    2026-08-15の流出分は所有者判断で対応済みとした（`force push`と`reflog expire`＋`gc`により
    ローカル・リモートの参照からは除去済み）。**次に同じことが起きたら、除去の可否と範囲を
    その時点で所有者へ確認すること**——リポジトリの公開範囲や流出内容によって判断が変わる。
21. **`MockWebServer.takeRequest()`を引数なしで呼ばないこと。**
    （2026-08-16追加。実際に4時間半を失った）引数なしの`takeRequest()`は**無期限にブロックする**。
    期待より実際のリクエストが少ないと、テストは「失敗」ではなく**ハング**する。
    変異注入でこれを踏み、原因究明に長時間を要した。CIで起きればジョブのタイムアウトまで
    無限に回り続ける。**`takeRequest(timeout, unit)`を使い、`null`が返ったら
    「何番目のリクエストが来なかったか」が分かるメッセージで失敗させること。**
    あわせて、**テストが想定時間を大きく超えたら異常を疑うこと**（本プロジェクトの全体テストは
    約48秒）。「待てば終わる」と考えて放置しない。
22. **実物のHTMLを取らずに「要素が無いはず」と決めて分岐条件を書かないこと。**
    （2026-08-17追加。進行指示14と同じ誤りの4度目）本棚0件の同期落ちで、
    「0件なら`select[name=otherbook]`が無いので`ShelfListParser`が失敗する」と推測して対処したが、
    実物は**selectが存在し、`value="0"`のプレースホルダoptionが1件入っていた**。
    `ShelfListParser`は成功し、次の`ShelfParser`が捕捉範囲の外で落ちていたため、
    **修正を出しても実機で直らなかった**。所有者から「0件の画面は新規作成画面と同じ」という
    正確な証言を得ていても、**その画面に何の要素があるかは実物でしか分からない**。
    解析の分岐条件を決める前に、対象画面のHTMLを1枚取得すること。
...
- 設計は `docs/design/calendar-due-dates.md`(所有者承認済み)。返却期限に到達する貸出がある日を、
  ...
  **カレンダーから貸出中へ遷移するとメンバー絞り込みは解除される**(2026-08-20レビュー反映。`LoansScreenController`は
  `@Singleton`で絞り込みが画面遷移をまたいで保持されるため、遷移先で対象行が絞り込みに隠れて0件と見える設計漏れが
  あった。カレンダーのドットが全員ぶんを表すことと一貫させる形で解除する対応にした。詳細は設計書§4.6参照)。
...
- 予約の取置期限(`Reservation.holdExpiryDate`)のカレンダー表示は今回のスコープ外(同じ仕組みで
  載せられるが別依頼とする)。
- 検証状況・未解決事項は設計書§7・§5.3を参照。**2026-08-21に所有者がCIのAPKで実機確認し、versionNameを1.4へ上げた**。
  確認できたのは「全体として問題なく動作」と「メンバー絞り込み中に遷移すると全員表示へ戻る」の2点で、
  見た目の細部(下記1・2)は不具合の報告が無かっただけであり個別に確かめた記録ではない。内訳は
  `docs/design/release-build.md`「1.4の実機確認記録」を参照。
...
## マイ本棚の編集（2026-08-09、段階6の配布準備完了・実サイト確認未実施）

**次の作業は、version 1.1のCI release APKで所有者が§9.5順に専用テスト棚を確認すること。**
開始5棚へ復元後、SHA・操作・期待・実測を記録する。技術設計の正本は`docs/design/bookshelf-editing.md`である。
...
- **完了（段階4）**: `BookshelfEditingUiController`、本棚画面の作成・名称変更・資料メモ編集・資料削除・
  本棚削除、明示最終確認、送信中の全入口無効化、結果表示を実装。Controller単体テストで境界値、
  二重送信防止、正確な削除対象を検証し、Android instrumented Composeテストでカード全体の詳細タップ、
  overflow非伝播、空棚表示を回帰条件化した。instrumentedテストはコンパイル成功、端末実行は未実施。
  全単体テスト成功、修正後Solレビューで重大・中程度の指摘なし
- **完了（段階5）**: 共有Controllerへ資料追加のメンバー・本棚・任意メモ入力、棚Flow購読、最終確認を
  追加し、全画面・ダイアログ双方の書誌詳細へ同じ入口を配線。毎回未選択、メンバー変更時の棚解除、
  棚読込み中/0件、最大長、確認中の対象消失・名称変更、StateFlow更新競合を単体テストした。Android
  instrumented Composeテストで追加入口と入力/最終確認を回帰条件化し、コンパイル成功、端末実行は未実施。
  全単体テスト成功、修正後Solレビューで重大・中程度の指摘なし
- **完了（段階6・配布準備）**: 主担当監査、対象テスト、全単体テスト731件、`assembleDebug`、
  `compileDebugAndroidTestKotlin`、秘密情報監査、Sol最終再レビュー（重大・中程度の指摘なし）。独立レビューで
  発見した、状態変更POST開始後cancelを`Unknown`へ固定する決定的境界と、確認スナップショット必須化・
  実サイト`before`不一致時の状態変更POST 0回停止を修正済み
- **未完了（段階6）**: 実サイトでの6操作・重複・`Unknown`非再送の確認、instrumentedテストの端末実行。
  version 1.1のCI release APKで所有者が§9.5順に専用テスト棚を確認し、開始5棚へ復元後、SHA・操作・期待・実測を記録する
- **未実施**: 実サイトへの書込み通信。段階2ではMockWebServer以外へPOSTしていない

### 実機検証で判明した未解決事項（2026-08-12）

- **メモ編集は実機再検証待ち**: commit `ccfbc7e` のCI release APKでは、資料メモ編集が
  「サイトの表示が変更された可能性」エラーで停止した。原因は、1段階目の`LBForm`が資料ごとに
  `[bookcmnt, eachcmnt, sortno, eachsortno]`を並べる一方、実サイトの確認`prevRequestForm`が
  項目名ごとに全資料分を再配列しており、項目名ごとの値列・件数は一致していたことだった。
...
### 本棚編集の統合と第3 POST修正（2026-08-12）

- 実サイトでは本棚名と全資料メモが同一の`LBForm`と「リストの更新」で送信される。アプリ側も
  `RenameShelf`と`UpdateItemMemo`を廃止し、単一の`EditShelf`へ統合した。本棚ヘッダーの
  「本棚を編集」画面で本棚名と全資料メモをまとめて編集し、1回の確認・1回の更新列で送信する。
- Gatewayは取得済み本棚の全資料を資料コード・タイトル・元メモ・DOM順で照合してから、同じフォームの
  `listname`と全`eachcmnt`だけを置換する。`bookcmnt`、本棚全体メモ、先頭表示設定、並び順、その他の
  controlはサイト取得値を維持する。成功判定は対象本棚の名前・全資料と他本棚不変を再取得で確認する。
- commit `3cbfbb2`の実機ログ（22:19更新）では、第1 POSTと`okCodes=OPACSDI011`付き第2 POSTはHTTP 200、
  応答にも完了メッセージと`WOpacSdiBookListDispAction.do`が存在したが、第3 POSTは0回で直後に再取得していた。
  原因箇所は`BookshelfCompletionFormParser`による第2 POST応答の拒否と確定した。
- 完了フォームでも、異なる項目名どうしのサーバー側再配列を許容するよう修正した。項目名集合、出現数、
  同名項目内の値列、prefix、資料行4項目、`okCodes=OPACSDI011`は引き続き厳密照合し、未知項目・値改変・
  同名項目内の並べ替えは拒否する。cross-name順序差から第3 POSTを1回送るGateway回帰テストを追加した。
- 関連単体テスト、debug/release全単体テスト、Android test Kotlinコンパイル、`git diff --check`は成功。
  Sol(low)独立レビューは重大・中・軽微の指摘なし。実機での統合編集と第3 POST成功は次APKで再検証する。
- 結果ダイアログ内テキストの長押しによるクラッシュは、この時点では未解決だったが次節で対応した。

### 本棚ダイアログの長押しクラッシュ回避とコピー導線（2026-08-12）

- 全画面の`SelectionContainer`は維持し、本棚編集で表示する5種類の`AlertDialog`だけを
  `DisableSelection`で除外した。別Windowのダイアログと親画面で選択状態を共有しないため、
  本文の長押しによる選択処理は起動しない。入力用`TextField`自身の編集・選択機能は維持する。
- 本棚操作のエラー／結果ダイアログに「メッセージをコピー」ボタンを追加した。コピー対象は画面に
  表示済みの本文だけで、タイトル、診断ログ、認証情報は含めない。コピーしてもダイアログは閉じず、
  既存の「閉じる」と独立している。
- 空文字のエラー／結果でもコピー操作後に表示を維持し、閉じる操作で消えるCompose回帰テストを追加。
  `compileDebugAndroidTestKotlin`、debug/release全単体テスト、`git diff --check`は成功した。
...
**リリース状況**: 未リリース。所有者判断（2026-08-14）により、次の機能追加とあわせて`versionName`を
1.2へ上げる。今回単独ではバージョンを上げていない。

#### 再発（2026-08-16 CI）と調査結果（2026-08-18）

`有効なルールとメンバーと既定館がある場合は自動予約をONに保存できる`が
CIで1回だけ`TimeoutCancellationException`で落ちた。**同じ内容のコードで直前のビルドは成功しており、
CI再実行でも成功した**ため、コードの退行ではない（`4990e32`を`reset --soft`でまとめ直した
`525d5b5`→`4990e32`で、ファイル内容は1バイトも変わっていない）。

**ベースライン測定（2026-08-18、ローカル20回連続）: 失敗 0/20（0%）。**
対象テストの実測所要時間は**毎回12〜18ms**で、5秒のタイムアウトに対し280〜400倍の余裕があり、
揺らぎもほぼ無い。**ローカルでは再現しない。**

このため**進行指示19が求める「修正前後の失敗率の比較」ができない**。
ベースラインが0%では、何を直しても改善を示せず、悪化しても気づけない。
**同じ調査を繰り返さないよう、この事実を記録しておく。**

実I/Oが関わるのは`settingsStore`（実DataStore）のみで、`AutoReservationRepository`は
インメモリのフェイクである。ただし**12〜18msが5秒を超えるには300倍以上の遅延**が要り、
「CIが遅い」だけでは説明しにくい。当該CI実行のログには
`The daemon has terminated unexpectedly on startup attempt #1`も出ており、
そのランナーが不安定だった可能性があるが**未検証**である。

**対応（所有者判断）**: タイムアウトを15秒へ引き上げ、タイムアウト時に
状態の要点を含む診断メッセージを出す。根本原因の解消ではなく先延ばしだが、
...
（独立レビューの中程度指摘）。**現状の残リスクは小さい**と判断して見送った。根拠は次のとおり。

- 4ソース（Room 2本・DataStore・StateFlow）はいずれも初回発行する実装である
- Flowが**例外**で止まる場合は、`SupervisorJob`下の未捕捉例外としてアプリごとクラッシュするため、
  「無言のハング」には至らない。ハングが現実化するのは「発行しないFlow」が将来混入した場合だけである

**発火条件（ここを変えるときは必ずセットで対応すること）**: `SettingsStore.settings`は
`dataStore.data.map{}`で`.catch`が無く、**DataStoreの読取りIOExceptionでクラッシュしうる**
（それ自体が潜在的欠陥。未修正）。これを`.catch { emit(既定値) }`で直すと、
**例外が出ずにFlowだけが終わる経路ができ、その瞬間にハングが現実化する。** そのときは
`init`の`collect`を`try/finally { initialStateLoaded.complete(Unit) }`で囲み、
購読が死んだら旧挙動（空のmembersで検証してエラー表示）へ縮退させること。

#### 残課題B（優先度低）: `mutateRules`のI/O区間がmutexを保持したまま

`mutateRules`は`replaceRules`と再取得のI/O区間全体を`rulesMutex`で保持しており、
`combine`購読も同じmutexを取るため、ルール保存中はメンバー変更・同期時刻・診断ログ件数の
表示更新がブロックされうる。初回読込側だけロック外へ出したので挙動が非対称になっている。

**`reloadRules`と同じ処理をそのまま適用してはならない。** `mutateRules`のmutexは
「連続保存で更新が失われない」不変条件を担保しており（`連続してルールを追加しても更新が失われない`
テストが守っている）、I/Oをロック外へ出すとその不変条件が壊れる。`reloadRules`でロック外へ
出せたのは、起動時1回きりで競合相手がいないからである。

直すなら**読み手側**である。`collect`が`rulesMutex`を取るのをやめ、ロック内で更新する
...
`SettingsUiState.initialized`は**UI側でどこからも参照されていない**（Controller内の定義と代入のみ）。
設定画面は初期化前でも既定値・空メンバーのまま描画されるため、**トグルは初期化前に押せる**。
これが原因1（初期化前のON操作で誤ったエラー）が実アプリで到達可能だった理由である。
表示を初期化完了までガードするかは別途判断すること（今回は挙動を変えていない）。
...
### レビュー指摘対応(2026-09-18、本セッションと独立レビューの指摘を1回でまとめて対応)

独立レビューは「指摘なし」(期待値の組み立て・単件追加の回帰・監視ジョブ・多重送信・完了コールバック・
状態更新・既存3画面の回帰すべて問題なし)。以下6件はいずれも軽微な指摘として対応した。

1. `BulkActionBar`の「⋯」を主ボタンの**左**から**右**へ移した(所有者承認済みプレビュー・設計書§5.1の図に
   合わせる)。`overflowActions`が空のときの描画は変わらない。
2. `BookshelfBulkAddProgressDialog`の件数を`minOf(completed + 1, total)`で頭打ちにし、最後の1件が
   終わった直後に「(total+1)件目/total件」と出ないようにした。
3. 本ファイルの「非モーダル」という誤記(旧2830行目付近・2855行目付近)を「閉じられないモーダル
   (`onDismissRequest = {}`。処理完了まで画面操作を遮る)」へ訂正した(上のセクションは訂正後)。
4. 設計書§5.4「確認待ちの間に一覧から消えた書誌は対象から外す」は本棚追加には適用しないと判断し、
   理由とともに書き換えた(コードは無変更。`dialog.items`をそのまま送る現状が妥当という結論)。
5. `confirmBulkAdd()`の連打で`addItems`が1回しか呼ばれないことをテストで固定した
   (`一斉追加の確定連打はaddItemsを1回しか呼ばない`、決定論的・実時間待ちなし)。
6. 設計書§8に、段階1独立レビューで判明した「POST後に想定外の例外が漏れた場合の分類」のリスクを追記した。

最新コミット: `b4a706e`。CI run 35362070288で`testDebugUnitTest`が実際に実行(タスク行にUP-TO-DATE/
FROM-CACHEの接尾辞が無いことで確認)され`BUILD SUCCESSFUL`。
...
## 実機確認の状況とVer.1.6（2026-09-20）

**更新（同日）: 下記「未確認」の6項目も所有者が確認し、versionName を1.6へ上げた。**内訳と、言及の無かった項目は `docs/design/release-build.md`「1.6の実機確認記録」を参照。以下は経緯として残す。

### 確認済み

- **一斉本棚追加**（`docs/design/bulk-bookshelf-add.md`）: §6.3 の6項目すべてを所有者がCIのAPKで確認済み。
  ただし項目1は「複数件を本棚へ一斉追加できた」との報告で、検索・新着の両方で試したか、公式サイト側で
  反映を確かめたかまでは報告に含まれない。

### 未確認（Ver.1.6へ上げる前に必要）

`docs/design/bulk-selection-followup.md` §7.3 の手動確認は**所有者がまだ試していない**（2026-09-20時点）。

1. 貸出中の「延長可能」ラベルが予約中の「順番待ち」と同じ見た目で揃うこと
2. 新着で選択後に絞り込むとバーの件数が表示中の数まで減り、戻すと戻ること
3. 選択したまま再検索・「更新」で警告が出て、「戻る」で実行されないこと（画面表示時・追加読み込み・0件では出ない）
4. 「今後は表示しない」と、設定「選択が解除される前に確認する」からの復活
5. 一斉直接予約（**実データを作る**）: 件数・メンバー・受取館の表示と、送信後の反映
6. 回帰: 予約中の一斉取消（**共通部品への移行後、一度も確認できていない**）、カートの一括削除と「カートを空にする」、貸出中の一斉延長と進捗表示

### 次にやること

上記の結果を受けたら、`docs/design/release-build.md` の履歴表に1.6の行と「1.6の実機確認記録」を加え、
`app/build.gradle.kts` の `versionName` を `"1.6"` へ上げる（1.4・1.5と同じ形式）。
1.6に含めるもの: 一斉操作の他画面展開（`bulk-selection.md`）、その追補（`bulk-selection-followup.md`）、
一斉本棚追加（`bulk-bookshelf-add.md`）。

## 蔵書検索: 検索結果と一時表示のリセット 完了（2026-09-20）

3段階の改修の第1段階（1. 本件、2. 長押しによる選択モード、3. 読書記録の複数選択）。設計書: `docs/design/search-result-reset.md`

### 現在の状態

- コミット: `85e7d7c`（設計書）、`ddec794`（実装）、`391a76d`（テスト修正）、`6de0837`（レビュー指摘対応）。すべてプッシュ済み
- CI: run 35459216355（`6de0837`）success。testDebugUnitTest と assembleRelease を含む
- `SearchScreenControllerTest` の @Test は30件（うち本件で追加したものは13件）
- versionName は1.6のまま。第1段階の実機確認が済んでから、第2・第3段階とあわせて版を上げるかを所有者と決める

### 完了範囲

- 新しい検索で、選択・カート追加の結果とエラーを常に消す。警告設定がオフのとき選択が残る不具合も、あわせて直した
- `resetOnLeave()`: 蔵書検索から他画面へ移るとき、`LibraryApp.navigateTo` 経由で呼ぶ。画面を回転しても呼ばない
- 直接予約の結果とエラー、処理中フラグは、再検索しても画面を移っても消さない（実サイトへの予約POSTの成否であるため）
- 一斉操作で対象を絞り込む処理（`currentTilcods`）を、確定操作の同期区間へ移した
- 世代番号（`searchGeneration`）で、キャンセル済みの検索が古い結果を書き戻すのを防ぐ

### レビュー

主担当と独立の Sonnet の2者でレビューした。指摘3件を1回の再委譲でまとめて対応済み。
- テスト8が応答待ちを再現していなかった → `runCurrent()` を追加した
- 直接予約のエラーが残ることを確かめるテストがなかった → 2件追加した
- 並行処理の窓 → 世代番号を導入した

### 未検証事項

- 実機確認（設計書 §6 の5項目）は未実施
- 世代番号で窓を塞ぐ効果は、単一スレッドのテスト用ディスパッチャでは再現できない。コードを読んで確認しただけである
- 新着資料画面にも、カート追加のメッセージが残り続ける同じ問題がある。対応するかは所有者の判断待ち

### 次にやること

- 所有者による実機確認（設計書 §6）
- 第2段階: 長押しによる選択モード。まずモックを作り、所有者に示す

## 新着資料画面のリセット 完了（2026-09-20）

`docs/design/search-result-reset.md` §8。第1段階の続きとして、所有者の指示で追加した。

- コミット: `80da524`（設計§8）、`4637748`（実装）、`a9565cf`（レビュー指摘対応）。すべてプッシュ済み
- CI: run 35476593583（`a9565cf`）success
- 巡回（更新）の3経路で、選択とカート追加のメッセージを常に消す。警告設定オフでも選択を解除する
- `resetOnLeave()`: 選択・カート追加の一時表示・絞り込み語を消す。**一覧と取得状態には触れない**
  （Room のキャッシュ由来であり、消すと戻るたびに公式サイトへの巡回が必要になる。蔵書検索との意図的な違い）
- 巡回のコルーチンには触れない。`displayRefreshMutex`・`refreshedThisSession` の整合を守るため
- レビュー指摘（独立レビュー）: 絞り込み語のリセットが combine 経由で非同期にしか `state.query` へ
  伝わらず、入力欄と一覧が食い違う窓があった → `_state.update` にも同期的に含めるよう修正した
- 未検証: 実機確認（設計書 §8.5 の3項目）

## 次の3段階（2026-09-20 時点）

所有者から追加の依頼を受けている。第1段階（検索・新着のリセット）は実装完了、実機確認待ち。

1. **第2段階: 長押しによる選択モード**（設計前）。モック `docs/mockups/selection-mode.html` を所有者へ提示済み。
   モックの末尾に挙げた5点（帯の位置、「すべて選択」の要否、案内の出し方、カート画面の扱い、
   選択0件時に選択モードを抜けるか）の回答待ち。
   併せて、状態表示（「延長可能」「順番待ち」等）を書誌名の下の行へ移す
2. **第3段階: 読書記録画面の複数選択**（設計前）。一括操作はカート追加・直接予約・本棚追加の3つ（所有者確定）。
   資料番号が空の行は選択対象外とする想定
3. **待ち時間の表示を全体のオーバーレイへ**（設計前、所有者の新規依頼）。現在の一斉本棚追加は
   閉じられないダイアログで画面を塞いでいる。これを画面上部か下部の小さい帯に替え、処理中も
...
## 長押しによる選択モード 段階2（予約中）完了（2026-09-20）

- コミット: `2abd660`（実装）、`0fa4a05`（テスト修正）、`37beabb`（レビュー指摘対応）
- CI: run 35512200895（`37beabb`）success
- 予約中画面に段階1と同じ方式を適用した。`statusLabel`（「受取可能」「順番待ち」等）を書誌名の下の行へ移した
- 一斉取消の完了でのみ選択モードを抜ける。単独取消の完了は選択に触れない（設計§3.5.1）
- 通信例外で終わったときは選択・選択モードを保つ。貸出中の一斉延長と同じ流儀
- **残課題C（lost update）を解消した**。`ReservationCancelUiController` の `_state.value = ...` 7箇所を
  `update {}` へ統一し、注意コメントも置いた。アプリの実コードに `_state.value = ...` は残っていない
  （`ReadingRecordsScreenController` の1箇所は単一のコレクタからの全置換であり、書き手が1つなので対象外。
  ただし第3段階で読書記録に選択機能を足すと書き手が増えるため、そのときに `update {}` 化すること）
- 未検証: 実機確認。特に `statusLabel` を移した行は、受取館・予約順位・取置期限が既に並んでおり、
  端末の幅によっては窮屈になる可能性がある（独立レビューの指摘。実機で見てから判断する）

## 長押しによる選択モード 段階3（蔵書検索・新着資料）完了 = 設計書の全範囲が実装完了（2026-09-20）

- コミット: `ffabc9c`。CI: run 35513220756 success
- レビュー: 主担当・独立レビューとも**指摘なし**
- 2画面のコントローラは自分で一覧を持つため、`enterSelectionMode(tilcod)` は引数で可否を受け取らず、
  資料番号が空・一覧に無い tilcod を自分で弾く（段階1・2は Screen から可否を渡す形）
- 選択モードを抜けるのは、一斉カート追加・一斉直接予約・一斉本棚追加の各完了、`resetOnLeave()`、
  新しい検索／巡回。通信例外では抜けない
- 先行改修（`search-result-reset.md`）の規則は維持（直接予約の結果とエラーは消さない、世代番号も健在）
- **未検証: 5画面ぶんの実機確認（設計書 §6 の15項目）**

### 次にやること

1. 所有者による実機確認（選択モード §6 の15項目。蔵書検索のリセット §6 の5項目も未確認のまま）
2. 読書記録の検索をAND検索にする（設計済み: `docs/design/reading-records-search.md`）
...
## 実機確認の結果と要修正（2026-09-20）

選択モードの5画面ぶんを所有者が実機で確認した。結果は `docs/design/selection-mode.md` §3.4・§3.7・§8 に反映済み（コミット `7d1e96c`）。

- 大半は問題なし。「受取可能」「順番待ち」の移動先の見た目も問題なしと確認された
- 要修正3件（実装を委任済み）
  1. 「選択解除」を一斉操作ボタンの左（通常時の案内と同じ位置）へ移す。`SelectionModeBar` は削除する
  2. 貸出中の「延長可能」は表示しない（延長ボタンで分かるため）
  3. **予約中・予約カートから開いた書誌詳細で長押しするとクラッシュする**。`Dialog` の中身が
     親ウィンドウの `SelectionContainer` に登録されることが原因との見立て。改修前から存在した
     不具合と判断した（§8.2 に根拠）

### 運用の反省

**実機確認に回す前に versionName を上げること。** 今回は 1.6 のまま据え置いたため、改修前の
ビルドと改修後のビルドがどちらも「1.6」と表示され、不具合がどちらに由来するかを所有者が
切り分けられなかった。

## 実機確認で出た3件の修正とVer.1.7（2026-09-26）

- コミット: `bb1ea88`（選択解除の位置）、`a805b03`（「延長可能」の削除）、`83ae670`（書誌詳細のクラッシュ）、
  `608124d`（設計書）。CI: run 36165440865 success
- レビュー: 主担当・独立レビューとも**指摘なし**
- クラッシュの対処は §8.3 の案1（`DisableSelection` の内側に新しい `SelectionContainer`）を採った。
  **実機での成否判定が未了**。落ちるようなら案2（ダイアログ内の選択を止める）へ切り下げる
- **versionName を 1.7 へ上げた**。前回の反省（版を据え置いたため改修前後を切り分けられなかった）を踏まえ、
  実機確認に回す前に上げた
- 1.7 に含まれるもの: 選択モードへの移行（4画面）、検索・新着のリセット、読書記録のAND検索、
  書誌詳細のクラッシュ修正、残課題Cの解消

### 次にやること

1. 所有者による実機確認（1.7）。「選択解除」の位置、「延長可能」が無いこと、書誌詳細で長押ししても
   落ちないこと・コピーができること
2. 読書記録の複数選択の実装（設計済み: `docs/design/reading-records-selection.md`）
3. 待ち時間の帯表示（設計前。確定事項は「画面の下に出す」）

## 読書記録画面の複数選択 完了（2026-09-26）

設計書: `docs/design/reading-records-selection.md`

- コミット: `4f81308`（実装）、`5242e5c`（レビュー指摘対応）。CI: run 36169420164 success
- 一斉カート追加・一斉直接予約・一斉本棚追加を追加。蔵書検索の作りを写した（抽象化はしていない）
- **選択キーは資料番号**。同じ資料の行は連動し、候補は `distinctBy` で重複を除く。件数も資料単位
- `_state` の全置換（`_state.value = newState`）を `update {}` の差し込みへ直した。
  これで絞り込みの更新で選択が消えない（lost update の最後の箇所）
- レビュー指摘2件を修正: 資料番号が無い行が通常モードでタップに反応していた回帰、
  `cartRepository` の no-op 既定引数（渡し忘れが静かに「成功」になる）を必須引数化
- versionName 1.7 に含まれる（`release-build.md` の1.7の行に追記済み）
- 未検証: 実機確認（設計書 §5 の8項目）

### 残っている技術的負債（記録）

- `NewArrivalsScreenController` に、カート系の no-op 既定引数が残っている（独立レビューの指摘）。
  DI が渡し忘れたときに静かに「成功」として振る舞う。今回は範囲外としたが、いずれ必須引数へ変えること

### 次にやること

1. 所有者による実機確認（1.7）。選択モード4画面＋読書記録、蔵書検索のリセット、読書記録のAND検索、
   書誌詳細の長押しでクラッシュしないこと
2. 待ち時間の帯表示（設計前。確定事項は「画面の下に出す」。対象は本棚追加だけでなく待ち時間の出る操作全般）

## 今後の改修予定（2026-09-26 時点。所有者と共有中）

未着手の予定をここへ集める。項目が増えたら追記する。

1. **待ち時間の帯表示**（設計前）。一斉本棚追加の閉じられないダイアログをやめ、画面の下に小さな帯で
   進捗を出し、処理中も画面を移れるようにする（同種の操作は禁止のまま）。本棚追加だけでなく、
   待ち時間の出る操作全般が対象。まず対象の操作を洗い出す（一斉本棚追加・一斉カート追加・
   一斉直接予約・一斉延長・一斉取消・カート確定・手動同期など）。確定事項は「帯は画面の下に出す」
2. **本棚内の書誌の表示順序**（設計前）。所有者の要望（2026-09-26）。
   - 本棚内の並び順を**公式サイトと揃える**。あわせて**アプリから順序を編集できる**ようにする
   - 所有者の観測（未確認の部分を含む）: 公式サイトの本棚編集画面に「並び順」の欄がある。
     N冊の本棚では1からNまでの一意の整数が割り当てられ、入力欄で編集できる。**詳しい挙動は未確認**
   - 実現方法の見当: アプリから並び順の値を指定して送信する。既存の本棚編集の送信経路
     （`BookshelfGateway` の二段階POST、期待値 `BookshelfMutationExpectation` による照合）に乗せる
   - 着手前に確かめること（いずれも未確認）:
     - 一覧・編集画面のHTMLで並び順の値がどのフォーム項目として出ているか
     - 1件だけ値を変えたときサイトが他の行をどう詰めるか（入れ替えか、押し出しか）
     - 重複した値・範囲外の値を送ったときの拒否のされ方
     - Roomに並び順の列を持たせる必要があるか（表示順の正本をどちらに置くか）
   - **実サイトへの送信を伴う調査は所有者自身が行う**（テスト目的のPOSTは走らせない方針）
3. （追記待ち）

## Ver.1.7 の実機確認完了と次の着手（2026-09-26）

- 所有者から「1.7の確認完了」との報告。内訳は `docs/design/release-build.md`「1.7の実機確認記録」に記載
- 書誌詳細の長押しクラッシュも直っている（§8.3 の案1で解決。案2への切り下げは不要）
- 設計書4本（`selection-mode.md`・`search-result-reset.md`・`reading-records-search.md`・
  `reading-records-selection.md`）の状態を「完了」へ更新した
- **次: 待ち時間の表示を画面下の帯へ移す。設計書 `docs/design/operation-progress-banner.md` を作成した。
  所有者へ確認中の2点（完了を帯で知らせるか、手動同期を帯に出すか）の回答を待って実装へ回す**

## 待ち時間の帯表示 完了・Ver.1.8（2026-09-26）

設計書: `docs/design/operation-progress-banner.md`

- コミット: `360fdf8`（設計）、`711a1a9`（実装）、`281d3a4`（レビュー指摘対応）。CI: run 36217188925 success
- 新規: `ui/components/OperationProgressBanner.kt`（`OperationKind`・純関数 `OperationProgressContentBuilder`・帯本体）。
  テスト18件
- `LibraryApp` が全操作の進行状態を購読し、純関数で1本の文言を決めて `bottomBar` の上段に描く。
  **新しいコントローラや状態の複製は作っていない**
- 一斉本棚追加の閉じられない進捗ダイアログを廃止（これが画面を移れない唯一の原因だった）。
  結果のダイアログは残し、どの画面でも出る
- 貸出中の画面内の一斉延長の進捗表示を削除（行の中の単独延長の表示は残す）
- レビュー指摘（主担当・独立レビューとも同じ）: 完了の帯の5秒タイマーが、途中で別の操作
  （とくに完了を帯に出さない手動同期）に割り込まれるとキャンセルされたまま復帰せず、古い完了文言が
  消えなくなる → 「完了の検出」と「消えるまでの時間」を2つの `LaunchedEffect` に分けて修正。
  経緯をコード内のコメントにも残した
- **versionName を 1.8 へ上げた**（実機確認に回す前に上げる方針）
- 未検証: 実機確認（設計書 §5 の10項目）

### 残っている予定

1. **本棚内の書誌の表示順序**（設計前）。「今後の改修予定」の節を参照。サイト側の挙動の確認が先
2. 技術的負債: `NewArrivalsScreenController` のカート系 no-op 既定引数（渡し忘れが静かに成功になる）

## Ver.1.8 確認完了・本棚0件の不具合修正・Ver.1.9（2026-09-26）

- 1.8 の実機確認: 10項目すべて確認済み（内訳は `release-build.md`「1.8の実機確認記録」）
- 1.8 の確認中に見つかった本棚の不具合2件を修正（設計書 `docs/design/bookshelf-create-from-empty.md`）
  - 本棚0件のメンバーで作成できない（`BS_PARSE_EXCEPTION`）: `6caee57`・`0b3d5a8`・`b33d8cd`
  - 最後の1件を削除すると「結果を確認できません」になり、以後操作できない: `b42de0d`
    （削除後の0件判定が、2026-08-17に訂正したはずの古い前提のまま残っていた。同期も0件を反映しなかった）
  - 0件の画面の判定を `EmptyBookshelfPage.matches`（実物HTMLで固定）に一元化し、作成前・削除後・同期で使う
  - レビュー: 主担当・独立レビュー。初版に3件の指摘（実物HTMLでのテスト不足ほか）→ 対応済み。最終版は指摘なし
- 帯の配色を greenBg / greenInk へ（`c5f212b`、所有者の要望）
- **versionName を 1.9 へ上げた**
- 0件からの作成は途中版のビルドで所有者が成功を確認済み。最後の1件の削除と、固まった状態からの復帰は未確認

### 検討中

- 処理中に書誌詳細の読み込みが待たされる件。閲覧の通信（ログインしない読み取り）を書き込みの列と
  分けて並行に通す案を所有者へ提示し、回答待ち。**いまの「全セッションで1本ずつ」は2026-07の実測
  （確認から確定まで時間が空くと予約が成立しない）に基づく規則であり、別セッションの並行通信の影響は
  未実測である。** 変更するなら所有者の実機確認が必須

## Ver.1.9 確認完了と、閲覧の列の分離（2026-09-27）

- 1.9 の本棚の修正は所有者が実機で確認済み
- **次: 閲覧の通信を書き込みと別の列で並行に通す。設計書 `docs/design/browse-lane.md`**。所有者の判断で着手
  （閲覧専用のセッションを順番待ちの仕組みごと分ける）。予約への影響は推定のため、所有者の実機確認が必須

## 閲覧の列の分離 完了・Ver.1.10（2026-09-27）

- コミット: `ff167c1`。CI: run 36254702068 success。レビュー: 主担当・独立レビューとも指摘なし
- `@BrowseSession` の閲覧専用 `LicsXpSession`（Cookie・`RequestRateLimiter` とも独立）を `RepositoryModule` で提供し、
  `LicsXpClient` の閲覧6操作だけがそれを使う。メンバーとしての通信は根セッションのまま
- 新規テスト `LicsXpBrowseLaneTest`（5件）。占有中でも閲覧が返ることを固定
- **versionName を 1.10 へ上げた**
- **未検証（最重要）: 一斉予約の最中に書誌詳細を開いても、予約が正しく成立すること**（設計書 §4）。
  別セッションの並行通信が予約を妨げないことは推定にとどまる。失敗したら閲覧セッションを根セッションへ戻せば取り消せる
