...
## マイ本棚の編集（2026-08-08、要件確定・技術設計未着手）

**次に設計セッションが着手する機能。** 機能要件の正本は`docs/spec.md`§3.13、サイト側の通信の
正本は`docs/site-research.md`§13である。**この2つを先に読むこと。**
...
- **未採取**: 資料削除と本棚削除の確認コード（`okCodes`の値）。作成`OPACSDI017`・
  更新`OPACSDI011`は採取済み。**ハードコードせず抽出＋想定値照合にする方針のため、
  実装前にこの2つを確定させること**（DevToolsのPayload、または1段階目応答の`okArray`代入から読める）
- **未着手**: 技術設計、Room・Repository・Gateway・UIのすべて
...
- 本棚削除の確認の具体（対象名と消える件数の見せ方）
- 追加・削除・メモ編集の成功後にローカルDBへ即反映するか、次の同期を待つか
  （予約取消はローカル即時削除の前例がある）
- 本棚の一覧・所属をRoomへどう持つか（現在の`shelf`は本棚単位の概念を持っていない）
- 予約系の共通書込ゲート（`ReservationOperationGate`）に参加させるかどうか。
  貸出延長は予約データに触れないため参加させていない
