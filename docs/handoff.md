...
必要かどうかは検証されていない**。いずれもブラウザ実測に基づく変更で害はないため、現状維持とする。
...
#### ブラウザとアプリで依然として異なる点（成立に影響しないことが実証された）
...
**2026-07-27追記: 上限に達したときのサイト応答は実機で確認済みとなった。** サイトは拒否理由を
表示せず予約確認画面を再表示する（site-research.md §6.4）。あわせて予約状況一覧が20件でも
...
サイトは拒否理由を一切表示せず予約確認画面を再表示するだけと判明した。また予約状況一覧は
予約上限20件でもページングされないと実測した（同 §9）ため、一覧照合に偽陰性は起きない。
