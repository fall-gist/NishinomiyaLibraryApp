...
### 4. 新着資料からキーワード一致で自動予約する（2026-07-29要件確定・設計着手）
...
- この時点では実装未着手。次に既存コードとの境界を確定した技術設計を作り、その後に実装する
...
## 新着キーワード自動予約（2026-07-29要件確定・設計前ハンドオフ）

機能要件の正本は`docs/spec.md`§3.11である。旧ロードマップの未決事項は所有者との対話で解決済み。
...
7. 端末通知は詳細を伏せ、ホームの一回限りのダイアログと最新履歴で結果を示す
...
既存の`NewArrivalRepository.refresh()`、`SyncWorker`、`ReservationGateway`、
予約/貸出/読書記録DAO、通知基盤、設定画面・ホーム画面の境界を確認し、Roomマイグレーション、
オーケストレータ、結果モデル、同時実行制御、UI状態の技術設計を作る。実装は設計確定後に行う。
