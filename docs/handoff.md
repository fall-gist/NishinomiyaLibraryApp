...
### 4. 新着資料からキーワード一致で自動予約する（2026-07-29要件・設計確定、実装未着手）
...
- 技術設計は`docs/design/new-arrival-auto-reservation.md`に確定した。次は同設計の実装順序に従って
  段階実装する
...
## 新着キーワード自動予約（2026-07-29要件・技術設計確定）
...
技術設計§12の順に、まずRoom v7→v8、ルールドメイン、照合純粋関数、制御記録を実装する。
その後、既存手動予約の結果解決を共通部へ抽出し、回帰テストが同一であることを確認してから
自動予約オーケストレータとUIを接続する。
