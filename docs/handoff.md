...
##### 予約取消側への横展開が可能になった（未着手）

実サイトで抽出方式（確認コード・送信先）が問題なく動くことを確認できたため、**据え置き中の
予約取消側の同型2件へ横展開してよい状態**になった。対象は前掲「既知の課題」の2件:

- `ReservationGateway.kt`の`requireNotMaintenance(stage1Html)`が1段階目POST後にあり
  `Failure(SITE_MAINTENANCE)`化される件（貸出延長§5.3と同型の是正が必要）
- `ReservationCancelConfirmationFormParser`の確認コード`OPACUSR001`のハードコード

**稼働中かつ実サイト検証済みの経路であるため、着手する場合は所有者の承認を得ること。**
...
- 第1次・第2次は所有者が実機で確認済み。**第3次（ドット縦位置とインデント）の実機での見た目確認は未実施。**
