...
### 4. 新着資料からキーワード一致で自動予約する（2026-07-29要件・設計確定、段階1〜8完了）
...
次は段階9の新着更新トリガーへの接続を実装する。通知、UI、Worker接続はまだ未実装であり、
段階8の自動予約経路による実サイト通信も未検証である。
