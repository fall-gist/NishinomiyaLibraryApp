...
- 未検証: 実サイトの読書履歴取得。認証情報(`LIB_CARD`/`LIB_PASS`)が必要なためリポジトリ所有者の実行待ち。`scripts/fetch-fixtures.sh`に初回`usrread`取得を追加済み。実行後は取得内容の個人情報マスキング確認が必要。
- 未検証: 実機/エミュレータでの読書記録の同期・表示確認。この環境には接続デバイス・AVDが存在しない。
