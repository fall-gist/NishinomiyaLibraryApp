...
### 4. 新着資料からキーワード一致で自動予約する（2026-08-01段階1〜11完了）
...
## 新着キーワード自動予約（2026-08-01段階1〜11完了）
...
段階12の実機回帰を一部実施した。コミット`e5280d8`のCI APKで、ルール適合時の1冊予約、予約枠を
埋める件数が一致した場合の予約、予約上限到達時の見送り件数と「全員の予約枠が不足していたため
見送りました」の表示、マスターOFF・個別ルールOFF時に予約されないこと、ホームの予約一覧、
再起動後の設定・ルール・履歴保持を所有者が2026-08-01に確認した。OFF時に予約POSTが一切
発生していないことの通信レベル確認、通知タップ→ホーム→結果ダイアログ、メンバー優先順の
フォールバック、実機上のHilt Singleton同一性は未検証である。

同じ実機確認で、全画面の最新履歴を最下部までスクロールしても「予約一覧を見る」を確認できない
視認性問題が判明した。導線をタイトル直下の固定領域へ移し、履歴本文だけをスクロール可能に修正した。
結果ダイアログ側の導線、ナビゲーション、確認済み処理の`runId`契約は変更していない。

手動予約・取消が共通書込ゲートを待つ際の専用表示は、待機状態の観測APIが存在しないため未実装である。
推測で待機表示を出すと誤表示になるため、段階12以降で`ReservationOperationGate`の観測契約を設計してから
接続する。
