...
第1段は実装済み。**ダブルレビューで判明した欠陥の修正（第1.5段、§10）が未着手**であり、
§6.1・§6.2・§6.3は実害があるため第2段より先に片付ける。
...
- `Success` — すべて完了
- `AppliedWithWarning` — **データの取り込みは完了したが、後処理（設定の書き込みまたは
  同期スケジュールの再構成）に失敗した**。「取り込みは完了しましたが、自動同期の再設定に
  失敗しました。設定画面で同期時刻を開き直してください」と案内する
