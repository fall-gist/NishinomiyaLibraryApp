...
作成日: 2026-08-29
状態: **完了（versionName 1.6）**。実装済み・所有者による実機確認は§8.3のとおり（2026-09-20）
...
| # | 項目 | 状況（2026-09-20時点） |
|---|---|---|
| 1 | A: 2件以上を選んで一斉延長し、成功した行の期限が更新され延長ボタンが消えること | **一部確認**（所有者が「複数選択での延長ができること」を確認。期限更新・ボタン消失までの言及は無い） |
| 2 | A: 処理中の進捗表示が出ること | **確認済み**（2026-09-20、回帰確認として所有者が実施） |
| 3 | C: 一部選択の削除、「カートを空にする」の両方 | **確認済み**（2026-09-20、同上） |
| 4 | D: 検索・新着それぞれから複数追加し、カート画面に正しいメンバーで入ること | **一部確認**（所有者が「複数一括カート入り」を確認。メンバーの正しさまでの言及は無い） |
| 5 | D: 既にカートにある書誌を含めて追加し、件数の文言が実態と合うこと | 未確認 |
| 6 | 予約中: 移行後も一斉取消が従来どおり動くこと（§4.1のリファクタの回帰確認） | **確認済み**（2026-09-20、同上） |

**「一部確認」は、所有者の報告にあった範囲だけを確認済みとしたものである。** 不具合の報告が無かったことを
もって全項目を確認済みとしない（`docs/design/release-build.md`「1.4の実機確認記録」と同じ扱い）。
特に**6は共通部品への移行に対する回帰確認**であり、動かしていなければ確認したことにならない。
