...
作成日: 2026-08-20
状態: **実装完了・所有者による実機確認済み（2026-08-21、versionName 1.4）**
...
8. メンバー絞り込みの解除（独立レビュー指摘への対応・2026-08-20追記）: 貸出中で特定メンバーに
   絞り込んだ状態でカレンダーへ移動し、**別のメンバーの本だけが期限の日**をタップする。遷移先で
   絞り込みが「ぜんいん」へ戻り、対象行が表示されて強調されること。§4.6の対応が実際に効いているかは
   この経路でしか確認できない（絞り込みが元から「ぜんいん」だと差が出ない）
