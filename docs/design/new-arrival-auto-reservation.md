...
最終更新: 2026-08-03
状態: 設計完了・段階1〜13完了・段階12の実機回帰を一部実施
...
- `ReservationSubmissionResolver`、共通書込ゲート、`AutomaticReservationCoordinator`、
  ...
  Android通知とホーム通知タップ遷移、設定・新着状態表示・ホームダイアログ・最新履歴UIまで実装済み
- 段階13完了時点の全単体テスト550件、実v7相当DBからv8へのRoomスキーマ検証が成功。
  ローカルAPKは作らず、実機検証にはGitHub CIのAPKを使用する
- コミット`e5280d8`のCI APKで、ルール適合時の自動予約、予約枠を埋める複数一致、予約上限時の
  見送り件数・理由表示、マスターOFF・個別ルールOFF時の予約抑止、予約一覧表示、再起動後の
  設定・ルール・履歴保持を所有者が2026-08-01に確認済み
- 実機で発見した最新履歴から予約一覧への導線の視認性を改善し、「予約一覧を見る」をタイトル直下へ
  固定して履歴本文だけをスクロール可能にした
- 共通書込ゲートへ操作種別だけの観測契約を追加し、手動カート予約・即時予約・予約取消が
  自動予約を待つ間だけ「自動予約処理の完了待ち…」を表示する段階13を実装済み。
  観測状態に資料・利用者・認証情報は含めず、永続化もしない
