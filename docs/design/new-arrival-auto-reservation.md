...
最終更新: 2026-08-01
状態: 設計完了・段階1〜11完了・段階12の実機回帰を一部実施
...
- 段階1〜11の全単体テスト542件と`assembleDebug`、実v7相当DBからv8へのRoomスキーマ検証が成功
...
- 手動予約・取消が共通書込ゲートを待つ際の専用表示は、待機状態の観測APIがないため未実装。
  推測状態は作らず、段階12以降で観測契約を設計して接続する
