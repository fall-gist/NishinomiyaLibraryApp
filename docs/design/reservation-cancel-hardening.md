...
**状態**: 実装済み・**実サイト確認済み（2026-08-06）**。受入条件1〜6をすべて満たした。
...
## 実装前に未検証だったリスクと、その解消（2026-08-06）

**予約取消stage1応答の完全なHTMLを保持していない**（実測物はJSフラグメントのみ）。
そのため「stage1 HTML全体で`okArray`への代入が1件だけである」ことは実装時点で**未検証**であった。
...
**解消（確認済み）**: 所有者が実機（CIのAPK）でアプリから予約取消を実行し、**取消が成立し、
続く自動非表示まで働くこと**を確認した。抽出方式が実サイトのstage1応答で空振りしないこと、
および方針Bで触れた2段階目POST後の経路（`resolveCancelAndHide`→非表示）が従来どおり
機能することを、アウトカムとして確定した。

**未記録**: この確認時のビルド元コミット符号（`BuildConfig.GIT_SHA`）は本書に残していない
（進行指示6が求める記録項目のうち、SHAだけが欠けている）。対象コードは`7d3954e`が最終である。

**引き続き未検証**: 「stage1 HTML全体で`okArray`代入が1件」という構造そのものは、完全なHTMLを
採取していないため直接には確認していない。今回確定したのは「実サイトで取消が成立した」という
アウトカムであり、構造の一次情報ではない。
