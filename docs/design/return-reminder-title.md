...
作成日: 2026-08-21
状態: **実装完了・所有者による実機確認は一部済み（2026-08-29、versionName 1.5）**。当日の文言は確認済み、期限超過の文言は未確認（§6）
...
3. **実機確認の残り（2026-08-29時点）**: 「今日が返却期限の本があります」は所有者が実機で確認済み。
   **「返却期限が過ぎた本があります」は未確認**（翌日以降、実際に超過する貸出が生じたときに確認できる見込み）。
   「明日返却の本があります」は文言・経路とも変更していないため据え置き（所有者判断）。
   「まもなく返却期限の本があります」は通知日数を2日前以上へ変更しないと出ないため、通常運用では確認機会が無い。
   内訳は `docs/design/release-build.md`「1.5の実機確認記録」を参照
