...
| 1.7 | `83ae670`（2026-09-26） | **長押しによる選択モードへの移行**。貸出中・予約中・蔵書検索・新着資料の4画面で、常時表示のチェックボックスをやめ、書誌タイルの長押しで選択モードに入る方式へ変更（選択モード中はタップが選択の切り替え）。あわせて蔵書検索・新着資料の検索結果と一時表示のリセット、読書記録の検索の単語AND検索化、予約中・予約カートから開く書誌詳細で長押しするとクラッシュする不具合の修正、`ReservationCancelUiController` のlost update修正（残課題Cの解消）。設計は`docs/design/selection-mode.md`・`search-result-reset.md`・`reading-records-search.md` |
