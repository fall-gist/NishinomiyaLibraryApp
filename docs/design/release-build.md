...
| 期限超過の貸出がある | 返却期限が過ぎた本があります | **未確認**（翌日以降、実際に超過が生じたときに確認できる見込み） |
| 前日（最短が明日） | 明日返却の本があります | **据え置き**（文言・経路とも変更しておらず、従来どおりで問題ないと所有者が判断） |
| 通知日数を2日前以上に設定 | まもなく返却期限の本があります | **未確認**（既定の1日前では出ない。通常運用では確認機会が無い） |

**未確認の2件を残したままバージョンを上げることは所有者の判断である。** 4区分のうち実機で確かめたのは
1区分のみであり、残りはユニットテスト（`NotificationPlannerTest` の8件と、通知タイトルの配線を見る
`AndroidAutoReservationNotificationTest` の2件）でのみ担保されている。

超過時の文言は、次に期限超過が発生した際に確認すること。**確認できたらこの表を更新すること。**
