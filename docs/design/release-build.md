...
| 1.10 | `ff167c1`（2026-09-27） | **閲覧の通信を書き込みと別の列で並行に通す**。検索・自動補完・貸出可否・書誌詳細・休館日・新着資料を、Cookieも順番待ちも独立した閲覧専用セッションへ分けた。一斉予約などの処理中でも書誌詳細が待たされない。予約・取消・延長・本棚・同期の手順と占有の範囲は不変。設計は`docs/design/browse-lane.md` |
