...
**状態**: 実装済み・**実機確認済み（2026-08-06）**。受入条件1〜3をすべて満たした。
所有者が実機（CIのAPK）で、警告表示・設定アプリへの遷移・**戻ったときに警告が消えること**
（`ON_RESUME`再評価）・許可済みでは警告が出ないことを確認した。
...
## 既知の制約: チャンネル単位の無効化は検出しない（2026-08-06、独立レビュー指摘により追記）

`AndroidNotificationSink.canPost()`は投稿可否を3条件で判定する。

1. ランタイム権限`POST_NOTIFICATIONS`
2. `NotificationManager.areNotificationsEnabled()`（アプリ全体の通知トグル）
3. 対象チャンネルの`importance != IMPORTANCE_NONE`（**チャンネル単位**）

本機能の`canReceiveNotifications()`は**1と2だけを見る**（当初は1のみだったが、レビュー指摘により2を追加した。
権限は許可のままアプリ全体の通知をオフにされた端末で、警告を出さないまま通知が届かなくなるため）。

**3は見ていない。** ユーザーが「返却期限リマインダー」「予約受取可能」のチャンネルを個別にオフにした場合、
本機能は`Granted`のまま警告を出さないが、そのチャンネルの通知は届かない。設計書の「スコープ外」に
通知の判定条件を挙げているとおり、チャンネル単位の粒度は本機能では扱わない。

対応するなら、どのチャンネルを見るか（片方でも無効なら警告か、両方無効のときだけか）と文言を
決める必要がある。**実機確認でこのギャップが実害として現れた場合に検討する。**
