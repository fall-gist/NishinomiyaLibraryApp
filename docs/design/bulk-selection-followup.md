...
作成日: 2026-08-30
状態: **完了（versionName 1.6）**。実装済み・所有者による実機確認は§7.3のとおり（2026-09-20）
...
**2つのボタンは同じ `BulkActionBar`（同じ行）に並べる。** 縦に2本のバーを積む実装は誤りである
（実装時に一度この誤りをしたが、独立レビューの指摘で修正した。§5.3-1参照）。

- 検索・新着: 「カートへ追加」と「予約する」の2ボタンを同じ行に並べる
  - 主アクション（`Button`、件数を付ける）は「予約する」。副アクション（`OutlinedButton`、件数を
    付けない）は「カートへ追加」。並び順は副→主（`OutlinedButton`の右に`Button`）
  - 「予約する」の色は**緑（`colors.green`）とし、赤（`colors.alert`）は使わない**（§5.3-2参照）
...
#### 5.3-1 `BulkActionBar`を2回呼んで縦に積んだ誤り（実装時に発生し、レビューで修正）

初回実装では`BulkActionBar`を2回呼び出し、「カートへ追加（N件）」「予約する（N件）」を縦に
2段のバーとして描画していた。`BulkActionBar`は`fillMaxWidth`の`Row`（縦padding 6dp）のため、
件数が二重に表示され、一覧の上を2段占有する見た目になっていた。これは本節が意図した
「1本のバーに2ボタン」と異なり、独立レビューの指摘で修正した。

対応として`BulkActionBar`に`secondaryActionLabel`・`secondaryEnabled`・`onSecondaryClick`
（いずれも既定値`null`/`false`）を追加し、指定したときだけ同じ`Row`に`OutlinedButton`（副）→
`Button`（主）の順で並べる形にした。指定しなければ既存呼び出し（予約中の一斉取消、予約カートの
一括削除、貸出中の一斉延長）の見た目は変えない。

#### 5.3-2 「予約する」の色を赤にしなかった理由

初回実装では強調のため`colors.alert`（赤）を使ったが、これはアプリの色の意味と衝突する。
赤は本アプリでは休館日・延滞・一斉取消など**破壊的・警告的な操作**に使われており
（予約中一覧の「一斉取消（N件）」ボタンが赤）、資料を確保する予約操作に使うと、同じ赤いボタンが
画面によって「取り消す」（予約中）と「予約する」（検索・新着）の逆の意味を持つことになる。

書誌詳細（`BookDetailView.kt`）の予約ボタンは緑の塗り`Button`であり、赤を使っていない。
一斉直接予約もこの前例に合わせ、`containerColor = colors.green`とした。
...
|---|---|---|
| 1 | E: 貸出中の「延長可能」ラベルが予約中の状態表示と揃って見えること | **確認済み**（揃っている） |
| 2 | F: 検索・新着それぞれから複数選択して直接予約し、サイトに予約が入ること（**実データを作る操作**） | **確認済み**（一斉直接予約ができた） |
| 3 | F: 受取館・メンバーが確認ダイアログの表示どおりに反映されること | 報告に含まれない（2の「できた」を超える言及は無い） |
| 4 | G: 選択したまま再検索して警告が出ること、「戻る」で検索されないこと | **確認済み**（動いている） |
| 5 | G: 「今後は表示しない」の後は警告が出ないこと、設定画面から戻せること | **確認済み**（設定が反映されている） |
| 6 | G: 絞り込みで隠れた選択がバーの件数から外れ、語を戻すと戻ること | **確認済み**（絞り込みが反映されている） |

**所有者の報告にあった範囲だけを確認済みとした。** 3（受取館・メンバーが表示どおりか）と、4のうち
「警告が出てはいけない場面（画面表示時・追加読み込み・選択0件）」は、報告に明示的な言及が無い。
...
4. `warnBeforeClearingSelection` をバックアップに含めないため、機種変更時は既定（オン）に戻る。
   意図した挙動である
5. **確認ダイアログの件数と実送信件数が食い違いうる**（独立レビューの指摘、2026-08-30・修正不要と判断）。
   ダイアログは開いた時点の件数を表示するが、実際に送るのは確定直前に一覧へ残っている対象だけである
   （確認待ち中に消えた対象は無視する。`bulk-selection.md` §4.3 と同じ規則）。
   モーダル表示中は利用者操作による一覧変更は起きないため、実際に食い違うのは新着の背景更新が
   同時に走った場合に限られる。**乖離は必ず「表示件数 ≧ 実送信件数」の安全側**であり、
   表示より多く予約することはない。結果ダイアログには実際の件数が出る。よって修正しない
6. **確定時にメンバーの在籍を再検証していない**（同上・修正不要と判断）。選択時には
   `state.value.members` に存在するかを検証している。確定時の再検証が無いのは既存の
   `confirmBulkCartAddition` と同じ流儀であり、本追補で新たに生じたリスクではない。
   直すなら一斉操作全体で揃えるべきで、この追補の範囲を超える
