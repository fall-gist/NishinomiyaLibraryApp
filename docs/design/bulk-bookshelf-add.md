...
|---|---|---|
| 1 | 検索・新着それぞれから2〜3件を選び、1つの本棚へ追加。本棚画面とサイトで追加を確認する | **確認済み**（検索・新着の両方から複数件を一斉追加できた。公式サイト側での確認までは報告に無い） |
...
**所有者の報告にあった範囲だけを確認済みとした。** 1は検索・新着の両方で確認済み（2026-09-20追記）。
公式サイト側で反映を確かめたかは報告に含まれていない。
