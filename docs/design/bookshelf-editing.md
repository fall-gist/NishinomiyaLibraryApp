...
最終更新: 2026-08-12
状態: **段階1〜5完了。段階6は実サイトで資料メモ編集・本棚名変更の成立を確認済み**
（他の操作の実サイト確認とinstrumentedテストの端末実行は未実施。詳細は§10 段階6）
...
**実サイト確認の進捗（2026-08-12、所有者が実機で実施）**:

- **確認済み**: 資料メモの編集（1書誌単独、および本棚名＋2書誌メモの同時編集）が実サイトへ反映される。
  この過程で、サイトの`changcmnt`が`bookcmnt`(hidden)と`eachcmnt`(textarea)の両方へ新値を入れることが
  判明し、`eachcmnt`だけを更新していた実装を修正した（`docs/site-research.md`§13.9）
- **確認済み**: 本棚画面の行レイアウト、3点リーダの表示位置、資料削除導線の編集ダイアログへの集約
- **未実施**: 本棚の作成・削除、資料の追加・削除、重複追加、`Unknown`非再送の実サイト確認
- **未実施**: instrumentedテストの端末実行、および結果ダイアログの案内文言（2026-08-12に3分類へ整理）の
  実機表示確認
