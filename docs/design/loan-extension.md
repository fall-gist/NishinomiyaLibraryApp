...
- 2段階目: `prevRequestForm`のDOM順controls＋末尾に`OK_CODES_NAME`＝抽出した確認コードを追加して
  ...
  **`action`属性の存在を必須条件にしてはならない**（実サイトで必ず停止するため。予約取消の
  ...
  - **段階3への申し送り**: 現在の`LoanExtensionConfirmationFormParser`はJS代入から抽出した送信先を
    検証に使うだけで、戻り値へ載せていない。Gatewayが送信先を自前の定数で持つと、将来パーサ側の
    許容条件を変えたときに検証した値と実際の送信先が食い違い得る。段階3では
    **パーサが検証した送信先をそのまま受け取って送る**形にすること（`LoanExtensionConfirmationForm`へ
    送信先を持たせる小さな変更で足りる）。
