...
最終更新: 2026-08-05
状態: **全7段階実装完了・実サイト一連確認成功**（2026-08-05）。§9の論点3件は2026-08-04に所有者が裁定済み。
実サイト確認の結果は`docs/handoff.md`「5. 貸出中画面からの貸出延長」の「段階7 実サイト一連確認」を参照。
...
- `OK_CODES_NAME`変数と確認コード値（実測`OPACUSR005`）は、予約取消の`OK_CODES_NAME`抽出と同じ
  ...
  確認コード値は`okArray[okArray.length] = "OPACUSR005";`という代入としてHTML内に実在するため、
  抽出は可能である（フィクスチャ`usrlend_extend_confirm.html`で確認済み）。**抽出値が1件でない、
  または想定値`OPACUSR005`と一致しない場合は送信前に停止する。**
  - この要件を守る理由は、**アプリが「表示されていない問い」に盲目的にOKを返すのを防ぐ**ためである。
    サイトが延長時に別の確認（例「この資料には予約が入っています。よろしいですか？」＝別コード）を
    出すよう変わった場合、コードをハードコードしていると、その別の問いに対して`OPACUSR005`を送る。
    二段階確認という安全機構の意味が失われる。
  - 段階3のレビューで、実装が`OK_CODE = "OPACUSR005"`のハードコード定数になっていることが判明した
    （既存の`ReservationCancelConfirmationFormParser`の流儀を踏襲したもの）。所有者裁定
    （2026-08-05）により、**貸出延長側のみ抽出方式へ修正し、予約取消は既存動作のまま据え置く**
    （稼働中かつ実サイト検証済みの経路に手を入れる回帰リスクを避けるため）。予約取消側は
    `docs/handoff.md`へ既知の課題として記録する。
...
### 5.3 1段階目と2段階目の間で停止した場合の結果（2026-08-05 所有者裁定）

段階3のレビューで、**§6の「POST前に確定した失敗」の「POST」がどちらの段階を指すか**が曖昧であることが判明した。
1段階目POSTは送信済みだが2段階目を送っていない状態（確認フォームのパース失敗、送信先検証失敗、
1段階目応答がメンテナンス画面）を、`Failure`とも`Unknown`とも読めてしまう。所有者裁定により
**次の3分岐に統一する**。

1. 一覧を再取得し、対象`tilcod`の返却期日が送信前より後へ変化していれば`Extended`
   （1段階目に副作用が無いことは未実測であるため、変化していたら正直に成功として扱う）
2. 再取得・解析に成功し、返却期日が変化していなければ`Failure(SITE_RESPONSE_CHANGED)`
   （2段階目を送っていないと確定できるため。§6の`Failure`は「**状態変更POST＝2段階目**を送る前に
   確定した失敗」を意味すると解釈する）
3. 再取得できない・対象行が1件でない・返却期日を解析できない場合は`Unknown`
   （メンテナンス画面を検知した場合は再取得も失敗するため、自動的にここへ倒れる）

**§5.2の「2段階目送信後」の意味論はこの裁定で変わらない。** 2段階目を送った後に返却期日が
変化していなければ、サイトの拒否と通信の取りこぼしを区別できないため従来どおり`Unknown`である。

この分岐は1段階目POST直後のすべての検証に一律で適用すること。**同じ`stage1`応答に対する検証の
一部だけが例外を素通しして`Failure`になる非対称を作ってはならない**（段階3の初回実装で、
メンテナンス検知だけが`try`の外にあり`Failure(SITE_MAINTENANCE)`へ倒れる欠陥が実際に発生した）。
...
### 6.1 延長成功後のローカル反映（2026-08-05 追記。設計の記載漏れを補うもの）

自動同期は1日1回であるため、何もしなければ**延長に成功しても貸出中一覧は古い返却期限を表示し続ける**。
利用者が「延長できていない」と誤解して再操作する恐れがあるため、成功時はローカルへ反映する。

- `Extended(newDueDate)`のとき、**Repository層**が当該`memberId`＋`tilcod`のRoom行を
  `dueDate = newDueDate`・`extendable = false`へ更新する。
- `newDueDate`は**サイトの一覧を再取得して読み取った実値**であり推測ではない。
  `extendable = false`は「延長成功後に対象行はボタンを失う」実測（`docs/site-research.md`§10）に基づく。
- 対象行が1件でない場合は**更新しない**（`docs/design/loan-extension.md`§4.3のフェイルクローズと同じ考え方）。
- `Unknown`・`Failure`のときは**一切書き換えない**。成否不明の状態をローカルへ書くと、次の同期まで
  誤った期限を表示することになる。
- 全メンバーの再同期は行わない。1件の書込みで足りるうえ、明示操作のたびに全同期を走らせると
  「1日1回」の方針と矛盾するため。

**代替案として検討し不採用にしたもの**: 成功後に当該メンバーの貸出同期を回す。単一の真実の源を
保てる点で理屈は綺麗だが、通信が増え、同期経路（`StatusRepositoryImpl`等）へ手を入れる必要が生じる。
すでにサイトの一覧から実値を読み取っている以上、その値を書くだけで足りる。
